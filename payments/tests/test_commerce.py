...
@pytest.mark.django_db
def test_site_export_includes_the_full_commerce_record():
    from sites.exports import site_export

    _, site, _, _, ticket_type, registration = commerce_fixture()
    ticket_type.refund_policy = "Refunds up to 48 hours before the event."
    ticket_type.save(update_fields=("refund_policy", "updated_at"))
    order, _ = reserve_ticket_order(registration=registration, ticket_type=ticket_type)
    mark_order_paid(
        order.id,
        stripe_object={"object": "payment_intent", "id": "pi_export"},
    )
    order.refresh_from_db()
    prepare_refund(
        order=order,
    ...
    export = site_export(site)

    # An order total alone is not a usable financial record. A subscriber
    # leaving needs what was sold, what each order contained, who holds
    # admission, and what money came back.
    assert export["ticket_types"][0]["name"] == "General admission"
    assert (
        export["ticket_types"][0]["refund_policy"]
        == "Refunds up to 48 hours before the event."
    )
    assert export["order_lines"][0]["order_id"] == order.id
    assert export["order_lines"][0]["line_total_cents"] == order.total_cents
    # One line covering the attendee and their guest issues two tickets.
    assert len(export["tickets"]) == export["order_lines"][0]["quantity"] == 2
    assert all(ticket["display_code"] for ticket in export["tickets"])
    assert export["refunds"][0]["reason"] == "Event canceled"
    assert export["disputes"] == []

    # The audit trail reads forward and shows how the money actually moved.
    history = export["financial_history"]
    assert [entry["event_type"] for entry in history] == [
        "order.created",
        "payment.succeeded",
        "refund.requested",
    ]
    assert history[0]["details"]["total_cents"] == order.total_cents
    assert history[-1]["refund_id"] is not None
    assert history == sorted(history, key=lambda entry: entry["created_at"])


def _stripe_identifier_fields(model):
    """Every Stripe identifier the model stores, ignoring amount fields."""
    return {
        field.name
        for field in model._meta.get_fields()
        if field.name.startswith("stripe_") and isinstance(field, models.CharField)
    }


@pytest.mark.django_db
def test_export_carries_every_stripe_identifier_it_stores():
    from sites.exports import site_export
    ...
    # A subscriber reconciling against their own Stripe dashboard needs the
    # join keys. Anything the platform stores, the export hands back.
    for section, model in (
        ...
        assert export[section], f"{section} had no rows to check"
        exported = set(export[section][0])
        missing = _stripe_identifier_fields(model) - exported
        assert not missing, f"{section} drops {sorted(missing)}"

    # member_subscriptions.plan_id used to point at a plan the export omitted.
    plan_ids = {row["id"] for row in export["membership_plans"]}
    assert all(row["plan_id"] in plan_ids for row in export["member_subscriptions"])
