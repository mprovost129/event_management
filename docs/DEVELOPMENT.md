...
Production session and CSRF cookies are host-only (`SESSION_COOKIE_DOMAIN`/`CSRF_COOKIE_DOMAIN` unset) and must stay that way: every self-serve site gets an instantly-verified `*.gatherhqs.com` host (`sites.services.create_subscriber_site`), so scoping the cookie to the parent domain would hand every tenant subdomain the platform session of every other tenant. `SiteResolutionMiddleware` redirects the `www.` control-host alias onto the canonical `PLATFORM_DOMAIN` host (GET/HEAD only) so a host-only cookie still survives normal browser navigation between the two. One consequence: an authenticated site administrator browsing their own public tenant site is no longer recognized there, so the staff-only **Back to dashboard** navigation control (`sites.context_processors.site_management`) never renders on a live tenant page — it's inert rather than removed, in case a future cross-domain handoff (e.g. a short-lived signed token) replaces it. `python manage.py check --deploy` enforces the host-only cookie via `platform.E034`/`E035`.
