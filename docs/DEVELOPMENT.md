...
Authoritative consolidated reports are available at `/sites/{site-id}/reports/`. Subscriber admins can download an audited JSON data export from the site dashboard; site managers cannot export. The export includes site content, contacts, memberships, events, registrations, participants, attendance history, reviews, and the full commerce record (ticket types, orders, order lines, tickets, refunds, disputes, the per-order financial audit trail, membership plans, subscriptions, and payments) including the Stripe identifiers needed to reconcile against the subscriber's own connected-account dashboard No Stripe API keys, webhook secrets, or raw payment payloads are included. Transient inventory holds are excluded because they expire and carry no meaning after the fact.
